...
let string_in_list str lst =
  if not (List.exists (fun s -> s = str) lst) then
...
let check_if_in_alphabet str alphabet =
  if not (List.exists (fun s -> s = str) alphabet) then
    failwith (Printf.sprintf "The element '%s' not in alphabet" str)

let verify_machine machine =
  check_single_char_list machine.alphabet "alphabet";
  check_single_char machine.blank "blank";
  check_if_in_alphabet machine.blank machine.alphabet;
    ...
    (fun state transitions_list ->
      ...
      List.iter
        (fun t ->
          ...
          check_if_in_alphabet t.read machine.alphabet;
          check_if_in_alphabet t.write machine.alphabet;
...
let verify_tape input alphabet =
  ...
  in
  List.iter (fun s -> check_if_in_alphabet s alphabet) strings
